export default class Player {
  constructor(id, name, skinPath, coord) {
    ...
    this.deathSpriteDuration = 12;

    this.previousX = this.coordX;
    this.previousY = this.coordY;
    this.renderX = coord[0];
    this.renderY = coord[1];
  ...
  interpolate(alpha) {
    this.renderX = this.previousX + (this.coordX - this.previousX) * alpha;
    this.renderY = this.previousY + (this.coordY - this.previousY) * alpha;
  }
  ...
  updatePlayer(updateData) {
    ...
    [this.coordX, this.coordY] = updateData.position;
  }
}
